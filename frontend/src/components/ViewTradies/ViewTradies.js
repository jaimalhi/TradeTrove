...
function ViewTradies() {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-white px-4 py-8 pt-[100px]">
            <h2 className="text-center text-dark-green mb-10 text-3xl font-bold">List of Contractors</h2>
                ...
                {tradies.length > 0 ? (
                    tradies.map((tradie) => (
                        <div key={tradie.uid} className="bg-white p-4 rounded-lg shadow-md mb-6">
                            ...
                            <p className="mb-1">Experience: <span className="font-medium">{tradie.years_experience} years</span></p>
                            <p>Skills: <span className="font-medium">{tradie.skills.join(', ')}</span></p>
                        </div>
                    ))
                ) : (
